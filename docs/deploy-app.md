...
5. Deploying the infrastructure required for the app (see section ["Deploying infra from CI/CD"](#deploying-infrastructure-from-cicd))
6. Grant access to the teams members to the resources in Azure for the environment (see section ["Granting access to Azure resources"](#granting-access-to-azure-or-power-bi-resources))
...
1. Modify product conventions to customize azure resource whose names are globally unique, or otherwise different in your own subscription:
   ...
   2. set `CompanyName` (line 38) to make it globally unique (eg change `CLC` to your initials)
...
3. Setup shared infrastructure:
   ```pwsh
   ./tools/infrastructure/provision-shared-services.ps1 -InfA Continue -EnvironmentType prod -Login
    ````
